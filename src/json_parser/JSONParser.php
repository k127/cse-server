...
class JSONParser {
	...
	public function parse($tokenId, $token) {
		if(!$this->validateToken($tokenId)) {
			throw new JSONParserException($this, 'Invalid syntax: expected another token');
		}
	...
	public function parseDocument($file) {
		...
		if(is_null($fileResource)) {
			throw new JSONParserException($this, sprintf('Could not open resource %s in reading', $file));
		}

		// instanciate a lexer
		$lexer = new JSONLex($fileResource, $this);
